...
#### 1.2.2. 「文」の種類 - print文（文字列を出力するための文）
...
ソースコードの中に複数の文がある場合は、上から順に一つづつ実行します。
...
インタプリタは、それぞれの変数に入った値を次のように記憶していられます。プログラムの実行を始めたときには、変数は何も覚えていません。

変数に値を書き込んだ後だと、インタプリタの記憶は例えば次のようになります：
